...
then
    ...
    if [[ $filename == *winequality* ]]
    then
        sed '2d' $sqlname > tmp; mv tmp $sqlname
    fi
    if [[ $tablename == *monk* ]]
    then 
        sed 's/^\ //g;s/\ data_.*$//g' $sqlname > tmp; mv tmp $sqlname
    fi
    if [[ $tablename == *hypo* ]]
    then
    ...
    if [[ $tablename == *spect* ]]
    then
        sed 's/\r//g' $sqlname > tmp; mv tmp $sqlname
        echo '' >> $sqlname
    fi
fi
echo "\." >> $sqlname
